    ...
    try {
        ...
        const files = languages.map((lang) => {
            ...
            for (const line of textLines) {
                translatedContent = translatedContent.replace(line, translations[lang]?.[line] ?? line);
            }
            return {
        ...
        const prData = await createPullRequest(token, owner, repo, branchName, `Add multilingual documentation (${languages.join(', ')})`, `This PR adds documentation in ${languages.join(', ')} using Docify AI localization.`, files);
        res.json({
            success: true,
